...
try{
    ...
    # P23 Thetis Meters/Gadgets compatibility subsystem is applied only after the
    # complete verified SAFE P21 chain. Native FLEX-5000 hardware/audio/DSP
    # sources remain byte-for-byte protected.
    $p23Protected=@('Console\\FWC\\fwc.cs','Console\\FWC\\fwcatuform.cs','Console\\audio.cs','Console\\dttsp.cs')
    $p23Hashes=@{}
    foreach($rel in $p23Protected){$p23Hashes[$rel]=(Get-FileHash -Algorithm SHA256 (Join-Path $WorkRoot $rel)).Hash}
    & (Join-Path $PSScriptRoot 'Apply-PowerSDR-ThetisMeters-P23.ps1') -SourceRoot $WorkRoot
    & (Join-Path $PSScriptRoot 'Verify-PowerSDR-ThetisMeters-P23.ps1') -SourceRoot $WorkRoot
    foreach($rel in $p23Protected){
        $after=(Get-FileHash -Algorithm SHA256 (Join-Path $WorkRoot $rel)).Hash
        if($after -ne $p23Hashes[$rel]){throw "P23 native-source boundary changed: $rel"}
    }
...
"@
    ...
    try{
        ...
        $name='PowerSDR-SQ4KOU-FLEX5000-KE9NS-v2.8.0.336-P23-THETIS-METERS-1TO1.x86.msi'
