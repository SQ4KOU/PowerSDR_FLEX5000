...
try{
    ...
    & (Join-Path $PSScriptRoot 'Apply-PowerSDR-SQ4KOUSplash-P21-FINAL2.ps1') -SourceRoot $WorkRoot
    & (Join-Path $PSScriptRoot 'Verify-PowerSDR-SQ4KOUSplash-P21-FINAL2.ps1') -SourceRoot $WorkRoot

    # P22: isolated Meters/Gadgets layer. Hash native FLEX-5000 source before
    # and after the patch so the feature cannot silently alter radio/audio/DSP code.
    $p22Protected=@('Console\\FWC\\fwc.cs','Console\\FWC\\fwcatuform.cs','Console\\audio.cs','Console\\dttsp.cs')
    $p22Hashes=@{}
    foreach($rel in $p22Protected){$p22Hashes[$rel]=(Get-FileHash -Algorithm SHA256 (Join-Path $WorkRoot $rel)).Hash}
    & (Join-Path $PSScriptRoot 'Apply-PowerSDR-MetersGadgets-P22.ps1') -SourceRoot $WorkRoot
    & (Join-Path $PSScriptRoot 'Verify-PowerSDR-MetersGadgets-P22.ps1') -SourceRoot $WorkRoot
    foreach($rel in $p22Protected){
        $after=(Get-FileHash -Algorithm SHA256 (Join-Path $WorkRoot $rel)).Hash
        if($after -ne $p22Hashes[$rel]){throw "P22 native-source boundary changed: $rel"}
    }
...
"@
    ...
    try{
        ...
        $name='PowerSDR-SQ4KOU-FLEX5000-KE9NS-v2.8.0.336-P22-METERS-GADGETS.x86.msi'
