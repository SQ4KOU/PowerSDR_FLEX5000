...
function Replace-RegexOnce([string]$Text,[string]$Pattern,[string]$Replacement,[string]$Label) {
    ...
    if($matches.Count -ne 1) { throw "P17 regex anchor count=$($matches.Count): $Label" }
...
$console = Replace-RegexOnce $console '(?m)^(?<i>[ \\t]+)InitConsole\\(\\);[ \\t]*// Initialize all forms and main variables[ \\t]*
...
$console = Replace-RegexOnce $console '(?m)^(?<i>[ \\t]+)UpdateDisplay\\(\\); // ke9ns[ \\t]*
...
Stage 'PASS: passive P17 instrumentation installed; native PAL/FWC/Audio/display ordering unchanged'
 @'
${i}long sq4kouInitConsoleStart = SQ4KOUUIDiagnostics.OperationBegin("STARTUP", "INIT_CONSOLE", null);
${i}InitConsole(); // Initialize all forms and main variables
${i}SQ4KOUUIDiagnostics.OperationEnd(sq4kouInitConsoleStart, "STARTUP", "INIT_CONSOLE", null);
'@ 'InitConsole timing'
...
Stage 'PASS: passive P17 instrumentation installed; native PAL/FWC/Audio/display ordering unchanged'
 @'
${i}SQ4KOUUIDiagnostics.ProducerTick();
${i}UpdateDisplay(); // ke9ns
'@ 'RunDisplay producer counter'
