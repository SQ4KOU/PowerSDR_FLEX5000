...
try{
    ...
    # P25 native Thetis meter renderer dependencies are isolated to this worktree.
    & (Join-Path $PSScriptRoot 'Prepare-PowerSDR-ThetisMetersDirect-P25-Dependencies.ps1') -SourceRoot $WorkRoot
    ...
    # P25 native port: stable Thetis MeterManager/ucMeter/frmMeterDisplay direct port.
    ...
    foreach($rel in $p24Protected){
        ...
        if($after -ne $p24Hashes[$rel]){throw "P25 direct-port source boundary changed: $rel"}
    }
...
"@
    ...
    try{
        ...
        $name='PowerSDR-SQ4KOU-FLEX5000-KE9NS-v2.8.0.336-P25-DIRECT-THETIS-METERS.x86.msi'
