...
try{
    ...
    # Build and test the exact FlexMeters module that will be installed.
    $flexTestProject=Join-Path $PSScriptRoot 'meters-native\FlexMeters.Tests\FlexMeters.Tests.csproj'
    & $msbuild $flexTestProject '/m' '/t:Rebuild' '/p:Configuration=Release' '/p:Platform=x86' '/v:minimal'
    if($LASTEXITCODE -ne 0){throw "FlexMeters build failed rc=$LASTEXITCODE"}

    $flexTests=Join-Path $PSScriptRoot 'meters-native\FlexMeters.Tests\bin\Release\FlexMeters.Tests.exe'
    if(!(Test-Path $flexTests)){throw "FlexMeters.Tests.exe missing: $flexTests"}
    & $flexTests
    ...
    # Current native FLEX-5000 Meters/Gadgets integration: RX1 + TX/FWC.
    & (Join-Path $PSScriptRoot 'Apply-PowerSDR-FlexMeters-RX1.ps1') -SourceRoot $WorkRoot -FlexMetersDll $flexDll
    & (Join-Path $PSScriptRoot 'Verify-PowerSDR-FlexMeters-RX1.ps1') -SourceRoot $WorkRoot
...
"@
    ...
    try{
        ...
        $name='PowerSDR-SQ4KOU-FLEX5000-KE9NS-v2.8.0.336-FLEXMETERS-RX1-TX-TEST.x86.msi'
