...
try{
    ...
    & (Join-Path $PSScriptRoot 'Apply-PowerSDR-FastAudioExit.ps1') -SourceRoot $WorkRoot
    & (Join-Path $PSScriptRoot 'Apply-PowerSDR-AudioBeforeStandby.ps1') -SourceRoot $WorkRoot
    & (Join-Path $PSScriptRoot 'Apply-PowerSDR-ShutdownEvidence.ps1') -SourceRoot $WorkRoot
    & (Join-Path $PSScriptRoot 'Apply-PowerSDR-FastStateSave-P07.ps1') -SourceRoot $WorkRoot
    & (Join-Path $PSScriptRoot 'Apply-PowerSDR-DisplayPerformance-P08.ps1') -SourceRoot $WorkRoot
    & (Join-Path $PSScriptRoot 'Apply-PowerSDR-StableDisplayPacing-P13.ps1') -SourceRoot $WorkRoot
    & (Join-Path $PSScriptRoot 'Verify-PowerSDR-StabilityBaseline-P14.ps1') -SourceRoot $WorkRoot

    # P17 is diagnostics only. It brackets existing operations and writes telemetry
    # from a background writer/watchdog; it does not reorder PAL/FWC/Audio/DSP.
    & (Join-Path $PSScriptRoot 'Apply-PowerSDR-UIDiagnostics-P17.ps1') -SourceRoot $WorkRoot
    & (Join-Path $PSScriptRoot 'Verify-PowerSDR-UIDiagnostics-P17.ps1') -SourceRoot $WorkRoot
    & (Join-Path $PSScriptRoot 'Apply-PowerSDR-CleanStartupPresentation-P18.ps1') -SourceRoot $WorkRoot
    & (Join-Path $PSScriptRoot 'Verify-PowerSDR-CleanStartupPresentation-P18.ps1') -SourceRoot $WorkRoot
    & (Join-Path $PSScriptRoot 'Apply-PowerSDR-ScreenBeforeAudio-P19.ps1') -SourceRoot $WorkRoot
    & (Join-Path $PSScriptRoot 'Verify-PowerSDR-ScreenBeforeAudio-P19.ps1') -SourceRoot $WorkRoot
    & (Join-Path $PSScriptRoot 'Apply-PowerSDR-OriginalFlexSplash-P20.ps1') -SourceRoot $WorkRoot
    & (Join-Path $PSScriptRoot 'Verify-PowerSDR-OriginalFlexSplash-P20.ps1') -SourceRoot $WorkRoot
    & (Join-Path $PSScriptRoot 'Apply-PowerSDR-SQ4KOUSplash-P21.ps1') -SourceRoot $WorkRoot
    & (Join-Path $PSScriptRoot 'Verify-PowerSDR-SQ4KOUSplash-P21.ps1') -SourceRoot $WorkRoot

    # P24: compile the original Thetis meter subsystem against the native FLEX-5000 base.
    # Thetis is used only as source/UI/renderer provenance. No Thetis radio backend is imported.
    $thetisRoot=Join-Path $HarnessRoot '.work\thetis-meter-ref'
    if(Test-Path $thetisRoot){Remove-Item $thetisRoot -Recurse -Force}
    & git clone --no-tags --filter=blob:none https://github.com/ramdor/Thetis.git $thetisRoot
    if($LASTEXITCODE -ne 0){throw "Thetis reference clone failed rc=$LASTEXITCODE"}
    Push-Location $thetisRoot
    try{
        & git checkout --detach 852bf0ef0b4f3886a13fc2846489aee16f361872
        if($LASTEXITCODE -ne 0){throw "Thetis reference checkout failed rc=$LASTEXITCODE"}
        if((git rev-parse HEAD).Trim() -ne '852bf0ef0b4f3886a13fc2846489aee16f361872'){throw 'Thetis reference SHA mismatch'}
    }finally{Pop-Location}

    $p24Protected=@('Console\\FWC\\fwc.cs','Console\\FWC\\fwcatuform.cs','Console\\audio.cs','Console\\dttsp.cs')
    $p24Hashes=@{}
    foreach($rel in $p24Protected){$p24Hashes[$rel]=(Get-FileHash -Algorithm SHA256 (Join-Path $WorkRoot $rel)).Hash}
    & (Join-Path $PSScriptRoot 'Apply-PowerSDR-ThetisMeters-P24-Native.ps1') -SourceRoot $WorkRoot -ThetisRoot $thetisRoot
    foreach($rel in $p24Protected){
        $after=(Get-FileHash -Algorithm SHA256 (Join-Path $WorkRoot $rel)).Hash
        if($after -ne $p24Hashes[$rel]){throw "P24 native-source boundary changed: $rel"}
    }
...
"@
    ...
    try{
        ...
        $name='PowerSDR-SQ4KOU-FLEX5000-KE9NS-v2.8.0.336-P24-NATIVE-THETIS-METERS.x86.msi'
    ...
    @(
      ...
      'WINDOW_STATE_PATCH=SQ4KOU_MAXIMIZED_RESTOREBOUNDS_P02_PREHIDE_CAPTURE_P07','SHUTDOWN_PATCH=SQ4KOU_GLOBAL_JOIN_BUDGET_REMOVE_REDUNDANT_SLEEPS_P03','AUDIO_EXIT_PATCH=SQ4KOU_TERMINAL_PA_ABORTSTREAM_P04','POWER_OFF_ORDER_PATCH=SQ4KOU_AUDIO_BEFORE_FWC_STANDBY_P05','SHUTDOWN_EVIDENCE=SQ4KOU_PHASE_AND_DB_TIMING_P06','STATE_SAVE_PATCH=SQ4KOU_INDEXED_SAVEVARS_P07','DISPLAY_PERF_PATCH=SQ4KOU_POINT_REUSE_NORMAL_PRIORITY_DEADLINE_PACING_WATERFALL_MEMMOVE_P08','WINFORMS_STARTUP_PATCH=DISABLED_P14','UI_FIRST_PATCH=DISABLED_P14','DISPLAY_STABILITY_PATCH=SQ4KOU_NATIVE_SLEEP_PACING_BELOWNORMAL_P13','STABILITY_BASELINE=SQ4KOU_P09_P10_REMOVED_P14','UI_DIAGNOSTICS=SQ4KOU_P17_NONBLOCKING_TIMELINE_UI_WATCHDOG_PAINT_RENDER_BAND_LAYOUT','STARTUP_PRESENTATION=SQ4KOU_P18_TRANSPARENT_UNTIL_FIRST_DISPLAY_PAINT_PLUS_IDLE','AUTOSTART_SEQUENCE=SQ4KOU_P19_UI_REVEAL_BEFORE_NATIVE_POWER_AUDIO','SPLASH=SQ4KOU_P21_APPROVED_600x384_BACKGROUND_DYNAMIC_STATUS_RETAINED_UNTIL_UI_REVEAL','METERS_GADGETS=SQ4KOU_P24_ORIGINAL_THETIS_SOURCE_PORT','THETIS_METERS_SOURCE_SHA=852bf0ef0b4f3886a13fc2846489aee16f361872','METERS_GADGETS_RENDERER=ORIGINAL_THETIS_METER_MANAGER_DIRECT2D','METERS_GADGETS_CONTAINER=ORIGINAL_THETIS_UCMETER','METERS_GADGETS_FLOATFORM=ORIGINAL_THETIS_FRMMETERDISPLAY','METERS_GADGETS_SCRIPT=ORIGINAL_THETIS_ROSLYN_METER_SCRIPT_ENGINE','METERS_GADGETS_DATA=FLEX5000_ADAPTER_PENDING_COMPILE_GATE','DTTSP_RUNTIME=KE9NS_2.8.0.329_RELEASE_BINARY','DTTSP_SOURCE_336_NOT_BUILT=TRUE',
