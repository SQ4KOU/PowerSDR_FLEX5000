...
function Stage([string]$s){ Write-Host "[SQ4KOU-P21-FINAL3-SPLASH-VERIFY] $s" }
...
foreach($p in @($splashCs,$splashResx,$splashDesigner,$consoleCs,$presentationCs)){ if(!(Test-Path -LiteralPath $p)){throw "P21 FINAL3 verify input missing: $p"} }
...
$m=$bgRx.Match($resx)
if(!$m.Success){throw 'P21 FINAL3 BackgroundImage resource missing'}
$payload=($m.Groups[1].Value -replace '\s','')
if($payload.Length -ne 17624){throw "P21 FINAL3 embedded base64 length mismatch: $($payload.Length)"}
...
if($sha -ne $expectedSha){throw "P21 FINAL3 embedded SHA256 mismatch: $sha"}

Add-Type -AssemblyName System.Drawing
$img=$null; $ms=New-Object IO.MemoryStream(,$bytes)
try{
    $img=[Drawing.Image]::FromStream($ms)
    if($img.Width -ne 600 -or $img.Height -ne 384){throw "P21 FINAL3 embedded dimensions invalid: $($img.Width)x$($img.Height)"}
}finally{
    if($img){$img.Dispose()}
    $ms.Dispose()
}
...
)){if(!$designer.Contains($token)){throw "P21 FINAL3 designer marker missing: $token"}}

foreach($forbidden in @('Properties.Resources.moonearth6','Properties.Resources.moonearth2','Random random = new Random()')){
 if($splash.Contains($forbidden)){throw "P21 FINAL3 KE9NS random splash code returned: $forbidden"}
}
if($console.Contains('Splash.CloseForm();')){throw 'P21 FINAL3 constructor closes splash before stable UI reveal'}
if(!$console.Contains('Splash.SetStatus("Finalizing Main Window")')){throw 'P21 FINAL3 finalizing marker missing'}
foreach($token in @('"FLEX_SPLASH_CLOSE_AT_REVEAL"','Splash.CloseForm();','form.Opacity = 1.0;')){
 if(!$presentation.Contains($token)){throw "P21 FINAL3 startup ordering marker missing: $token"}
}

Stage "PASS: SQ4KOU splash exact bytes/dimensions verified; P19/P20 reveal-before-audio preserved; SHA256=$sha"
